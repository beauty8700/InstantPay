...
// Login only needs username + password — separate schema avoids validation failure
const LoginSchema = z.object({
  username: z.string().email("Username must be a valid email"),
  password: z.string().min(1, "Password is required"),
});

// Update allows partial updates — all fields optional but at least one required
...
router.post("/signup", async (req, res) => {
  try {
    ...
    // Give every new user a random starting balance (₹1,000 – ₹10,000) for demo
...
// ── GET /api/user/bulk?filter=name  (public — needed for send-money search) ─
